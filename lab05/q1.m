...
T = readtable('notbsedata.csv');
T_matrix = T{1:94,2:11};
...
%Index as Market portfolio
T1 = readtable('bseindex.csv');
T1_matrix = T1{2:1229,2:2};
T1_return = price2ret(T1_matrix);
C1 = cov(T1_return);
m_ind = mean(T1_return);

figure(2)
beta(1:10) = (m(1:10)-u_rf)/(m_ind-u_rf);
myu_v(1:10) = u_rf + (u_m-u_rf)*beta(1:10);
plot(beta(1:10),myu_v(1:10),'-*');
title('Security market line of 10 stocks');
